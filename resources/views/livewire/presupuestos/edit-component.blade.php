...
<div class="container mx-auto">
    ...
    <form wire:submit.prevent="update">
                    ...
                    <div class="col-sm-10" wire:ignore.self>
                        <select id="producto" class="form-control seleccion" wire:model="producto">
                            ...
                            @foreach ($productos as $product)
                                <option value="{{$product->id}}">{{$product->cod_producto}} - {{ $product->descripcion }}</option>
                            @endforeach
                        </select>
                        @error('denominacion') <span class="text-danger">{{ $message }}</span> @enderror
                    </div>
                        ...
                        @foreach ($lista as $productoE => $cantidad)
                            @if($cantidad > 0)
                                <tr id={{$productos->where('id', $productoE)->first()->id}}>
                                    ...
                                    <td>{{$cantidad}}</td>
                                    <td>{{(($productos->where('id', $productoE)->first()->precio_venta) * $cantidad) }}€</td>
                                <tr>
                            @endif
                        @endforeach
                        <th></th>
        ...
        <div class="mb-3 row d-flex align-items-center">
            ...
            <div class="col-sm-10">
                <input type="number" wire:model="precio" class="form-control" name="precio" id="precio" disabled>
                @error('fecha_emision') <span class="text-danger">{{ $message }}</span> @enderror
            </div>
        </div>
